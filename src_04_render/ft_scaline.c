...
/*   Updated: 2023/02/28 19:16:33 by kyoulee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	int index;
	index = (x * 4) + (y * param->mlx.size_line);
	if (0 <= index && index <= (WIN_WIDTH * 4) * (WIN_HEIGHT *  param->mlx.size_line))
		*(int *)(param->mlx.back_buffer + (x * 4) + (y *  param->mlx.size_line)) = color;
}
