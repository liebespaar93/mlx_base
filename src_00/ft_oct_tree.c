...
int	ft_next_oct_type(t_oct_tree *now, double *position, bool (*allow)[3])
{
	int	type;

	type = 0;
	(*allow)[0] = (position[0] - now->position[0]) > 0;
	(*allow)[1] = (position[1] - now->position[1]) > 0;
	(*allow)[2] = (position[2] - now->position[2]) > 0;
	if ((*allow)[0])
		type += 1;
	if ((*allow)[1])
		type += 2;
